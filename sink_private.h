...
typedef struct
{
    ...
    unsigned    :2;                             /* Unused */
    unsigned    BleAdvertisingFilter:6;         /* Advertising filter for BLE services */
...
typedef struct PIO_block_t
{
    ...
    BITFIELD    DeviceAudioActivePIO:8;
...
#define PanicUnlessRealloc(ptr, size) PanicNull(realloc(ptr, size))
